...
Install [docker](https://docs.docker.com/get-docker/) and [docker-compose](https://docs.docker.com/compose/install/) yourself, then run the following to set up first Odoo instance @ `localhost:10019` (default master password: `********`):

``` bash
curl -s https://raw.githubusercontent.com/Hootei/odoo-19-docker-compose/master/run.sh | bash -s odoo-one 10019 20019
```
and/or run the following to set up another Odoo instance @ `localhost:11019` (default master password: `********`):

``` bash
curl -s https://raw.githubusercontent.com/Hootei/odoo-19-docker-compose/master/run.sh | bash -s odoo-two 11019 21019
```
...
* Default database password (**admin_passwd**) is `*****`, please change it @ [etc/odoo.conf#L75](/etc/odoo.conf#L75)
